...
- 改进 Antigravity 安装体验：默认安装后即可获得决策评审 workflow，不再需要额外参数。
- 调整 Antigravity 2.x 的安装方式，优先使用官方 plugin 形态，减少不同环境下的识别差异。
- 保留对旧版 Antigravity 环境的兼容，但不再主动创建旧版目录。
- 文档推荐顺序调整为优先全局安装；仅当 IDE 无法识别全局安装时，再使用 workspace 安装作为 fallback。
