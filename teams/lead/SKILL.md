...
- `Taleb`：尾部风险、脆弱性、冗余和可失败试验。
- `Xunzi`：训练纪律、环境塑造和长期习惯。

## 组合依据

- **结果/责任层**：`Drucker`、`Simon`、`Weber`，用于明确外部结果、责任边界、流程成本和制度合法性。
- **偏差/激励层**：`Kahneman`、`Hanfeizi`、`Munger`，用于检查判断偏差、规则漏洞、激励扭曲和能力圈。
- **系统/自动化层**：`Meadows`、`Turing`、`Godel`、`Shannon`、`Taleb`，用于检查反馈回路、自动化边界、规则完备性、沟通失真和尾部风险。

## 路由

| 任务 | 必选 | 可选 |
| --- | --- | --- |
| 技术路线取舍 | Simon, Drucker, Turing, Newton | Godel, Taleb |
| 研发组织调整 | Drucker, Weber, Simon, Hanfeizi | Kahneman, Meadows |
| 质量债和架构债 | Meadows, Newton, Drucker | Taleb, Turing |
| 自动化/平台化 | Turing, Simon, Shannon | Godel, Drucker |
| 团队执行失真 | Weber, Hanfeizi, Shannon, Kahneman | Xunzi, Drucker |
| 交付节奏和流程 | Simon, Drucker, Meadows | Kahneman, Newton |
| 技术风险评审 | Taleb, Turing, Newton, Bayes | Godel, Munger |

## 方法

1. 用一句话重述技术或组织问题要改善的外部结果。
2. 区分技术问题、流程问题、激励问题、沟通问题和能力问题。
3. 选择 3 到 5 个视角，每个视角只给一个关键判断。
